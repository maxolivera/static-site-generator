    ...
    for node in old_nodes:
        ...
        while j < len(text):
            if not images: # All images have been extracted
                new_nodes.append(text[j:len(text)], "text")
                break
            ...
            if char == "!" and text[j+2:j+len(images[0][0])+2] == images[0][0] and text[j+len(images[0][0])+4:j+len(images[0][0])+4+len(images[0][1])] == images[0][1]:
...
def split_nodes_link(old_nodes):
    ...
    for node in old_nodes:
        ...
        while j < len(text):
            ...
            if char == "[" and text[j+1:j+len(links[0][0])+1] == links[0][0] and text[j+len(links[0][0])+3:j+len(links[0][0])+3+len(links[0][1])] == liks[0][1]:
                if current_segment: # If there is anything before the link, append it
                    new_nodes.append(TextNode(current_segment, "text"))
                
                current_link = images.pop(0)
                
                # Append the TextNode of this link

                new_nodes.append(TextNode(current_image[0], "link", current_image[1]))
